...
import SessionsViewSelector from "./SessionsViewSelector";
...
function Sidebar({
  ...
  // Sessions list hook for Session View
  const {
  ...
  const getAllSessions = (project) => {
    ...
    const codexSessions = (project.codexSessions || []).map((s) => ({
      ...s,
      __provider: "codex",
    }));
  ...
  const deleteSession = async (projectName, sessionId, provider = "claude") => {
    ...
    try {
      console.log("[Sidebar] Deleting session:", {
        projectName,
        sessionId,
        provider,
      });
      ...
      console.log("[Sidebar] Delete response:", {
        ok: response.ok,
  ...
  // View mode and timeframe change handlers
  ...
  // Handle session select from SessionListView
  const handleSessionListSelect = (session) => {
    // Find the project for this session
    const project = projects.find((p) => p.name === session.project?.name);
    if (project) {
    ...
    <>
      ...
      >
        ...
        <ScrollArea className="flex-1 md:px-2 md:py-3 overflow-y-auto overscroll-contain">
          {/* Session View - Flat list of all sessions */}
          {viewMode === "session" ? (
            <SessionListView
              sessions={sessionsList}
              selectedSession={selectedSession}
              onSessionSelect={handleSessionListSelect}
